...
const PRESET_LOCATIONS=[
  ...
  {label:"을지로입구역",sub:"서울 중구",x:126.9822,y:37.5660},
  {label:"을지로3가역",sub:"서울 중구",x:126.9920,y:37.5662},
  {label:"군자역",sub:"서울 광진구",x:127.0796,y:37.5573},
  {label:"신당역",sub:"서울 중구",x:127.0145,y:37.5659},
  {label:"동묘역",sub:"서울 종로구",x:127.0152,y:37.5720},
  {label:"동대문역",sub:"서울 종로구",x:127.0093,y:37.5712},
  {label:"종로3가역",sub:"서울 종로구",x:126.9920,y:37.5710},
  {label:"경복궁역",sub:"서울 종로구",x:126.9760,y:37.5759},
  {label:"보문역",sub:"서울 성북구",x:127.0195,y:37.5802},
  {label:"안암역",sub:"서울 성북구",x:127.0296,y:37.5862},
  {label:"성수역",sub:"서울 성동구",x:127.0560,y:37.5445},
  {label:"뚝섬역",sub:"서울 성동구",x:127.0472,y:37.5475},
];

/* ── 내가 아는 맛집 목록 (위치별) ── */
const MY_PICKS={
  "을지로입구역":["pasta ta ta","몽중헌 페럼타워점","타마고"],
  "을지로3가역":["오봉집"],
  "군자역":["후룩 누들바 군자"],
  "신당역":["하오카오스탠 신당점","이공김밥 신당점","표주","신당동천팥죽"],
  "동묘역":["존앤마크 동묘","동묘마게트"],
  "동대문역":["빠뜨로"],
  "종로3가역":["비틀비틀","비틀스타코 서순라점"],
  "경복궁역":["서촌계단집","계미산장"],
  "보문역":["운치","루 브런치&양식"],
  "안암역":["야마토텐동"],
  "성수역":["르프리크 성수","HDD피자 성수점"],
  "뚝섬역":["데이릿"],
};
...
async function fetchMyPicks(locationLabel) {
  ...
  for (var i = 0; i < picks.length; i++) {
    try {
      var res = await fetch(API_BASE + "/api/kakao-places?type=keyword&query=" + encodeURIComponent(picks[i]) + "&size=1");
      if (res.ok) {
        var data = await res.json();
        if (data.documents && data.documents.length > 0) {
          var d = data.documents[0];
          results.push({
            id: "pick-" + i, name: d.place_name || picks[i],
...
/* ═══ PLACE ROULETTE ═══ */
function PlaceRoulette({rouletteItems,filteredPlaces,spinning,setSpinning,winner,setWinner,showOverlay,setShowOverlay,fortune,setFortune,charMood,setCharMood,tab,filterProps,loading,onRefresh,totalPages,currentPage}){
...
function App(){
  ...
  useEffect(function(){
    ...
    setLoading(true);
    setPageIndex(0);
    // API 결과 + 내 맛집 동시에 불러오기
    Promise.all([
      fetchPlaces(placeTab,coords.x,coords.y,radius),
      placeTab==="restaurant"?fetchMyPicks(location):Promise.resolve([])
    ]).then(function(results){
      var apiPlaces=results[0];
      var myPicks=results[1];
      // 내 맛집 중 API 결과에 이미 있는 것은 제외 (이름 기준)
      var apiNames=new Set(apiPlaces.map(function(p){return p.name;}));
      var uniquePicks=myPicks.filter(function(p){return !apiNames.has(p.name);});
      // 내 맛집을 앞에 넣고, API 결과를 셔플해서 뒤에 합침
      var shuffledApi=apiPlaces.sort(function(){return Math.random()-0.5;});
      var combined=uniquePicks.concat(shuffledApi);
      setAllPlaces(function(prev){var next={};next[placeTab]=combined;return Object.assign({},prev,next);});
      setLoading(false);
    });
  },[coords.x,coords.y,radius,mainTab,location]);

  var placeTab=mainTab==="restaurant"||mainTab==="cafe"?mainTab:"restaurant";
  var filteredPlaces=(allPlaces[placeTab]||[]).filter(function(p){
    return !excludedCategories.has(p.category);
  });
